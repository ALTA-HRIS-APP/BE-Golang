...
func TestGetAll(t *testing.T) {
	...
	t.Run("success get all karyawan",func(t *testing.T) {
		...
		srv:=New(repo)
		_,response,err:=srv.Get("token","9182930",inputParam)
		assert.Nil(t,err)
	...
	t.Run("success get all not karyawan",func(t *testing.T) {
		...
		repo.On("SelectAll","token",inputParam).Return(int64(56),returnData,nil).Once()
		srv:=New(repo)
		_,response,err:=srv.Get("token","9182930",inputParam)
		assert.Nil(t,err)
		assert.Equal(t,returnData[0].ID,response[0].ID)
	...
	t.Run("failed get all not karyawan",func(t *testing.T) {
		repo.On("SelectUserById","9182930").Return(returnUserNotKaryawan,nil).Once()
		repo.On("SelectAll","token",inputParam).Return(int64(0),nil,errors.New("failed get reimbursement")).Once()
		srv:=New(repo)
		_,response,err:=srv.Get("token","9182930",inputParam)
		assert.NotNil(t,err)
		assert.Nil(t,response)
